...
//
/**\class c4h::TestRelationsAndLinks
...
namespace c4h {
  class TestRelationsAndLinks : public edm::global::EDAnalyzer<> {
  public:
    explicit TestRelationsAndLinks(const edm::ParameterSet& iConfig);
    ~TestRelationsAndLinks() override = default;
  ...
  TestRelationsAndLinks::TestRelationsAndLinks(const edm::ParameterSet& iConfig)
  ...
  void TestRelationsAndLinks::analyze(edm::StreamID, const edm::Event& iEvent, const edm::EventSetup& iSetup) const {
    ...
    for (const auto& track : iEvent.get(trackCollectionToken_)) {
      if (track.getType() != expectedTrackTypeInTrackCollection_) {
        throw cms::Exception("TestFailure")
            << "TestRelationsAndLinks: expected track type " << expectedTrackTypeInTrackCollection_
      ...
      if (tracksInTrack.empty()) {
        throw cms::Exception("TestFailure")
            << "TestRelationsAndLinks: expected track to have a track in its relation but it has none";
      }
      if (tracksInTrack.front().getType() != expectedTrackTypeTrackInRelationOrLink_) {
        throw cms::Exception("TestFailure")
            << "TestRelationsAndLinks: expected track type " << expectedTrackTypeTrackInRelationOrLink_
    ...
    for (const auto& recoParticle : iEvent.get(reconstructedParticleCollectionToken_)) {
      ...
      if (tracksInRecoParticle.empty()) {
        throw cms::Exception("TestFailure")
            << "TestRelationsAndLinks: expected RecoParticle to have a track in its relation but it has none";
      }
    ...
    // Test a link collection by getting the MCParticle linked a track.
    for (auto const& link : iEvent.get(trackToMCParticleLinkCollectionToken_)) {
      ...
      if (mcParticle.getPDG() != expectedTestValueInMCParticle_) {
        throw cms::Exception("TestFailure")
            << "TestRelationsAndLinks: expected test value " << expectedTestValueInMCParticle_
  ...
  void TestRelationsAndLinks::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(c4h::TestRelationsAndLinks);
