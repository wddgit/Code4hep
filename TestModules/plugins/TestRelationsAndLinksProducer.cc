...
namespace c4h {
  ...
  void TestRelationsAndLinksProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
    ...
    if (linkToCollectionsMadeInModule_) {
      ...
      const auto& inputTrack = iEvent.get(tracksGetToken_).at(0);
      ...
      const auto& inputMCParticle = iEvent.get(mcParticlesGetToken_).at(0);
